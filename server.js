...
app.post("/api/generate", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[${req.requestId}] generate start clientId=${safeClientId} promptLength=${String(prompt).length}`
    );
    ...
    console.log(`[${req.requestId}] build project`);
    await runCommand("npm", ["run", "build"], { cwd: projectDir, requestId: req.requestId });

    const srcPrefix = `${safeClientId}/src`;
    const buildPrefix = `${safeClientId}/build`;

    console.log(`[${req.requestId}] upload src to s3://${bucket}/${srcPrefix}`);
    ...
    console.log(`[${req.requestId}] upload build to s3://${bucket}/${buildPrefix}`);
    ...
    console.log(`[${req.requestId}] upload ok src=${srcPrefix} build=${buildPrefix}`);
    ...
    if (workspaceDir && process.env.ROBOSITE_KEEP_WORKDIR !== "true") {
      await fs.rm(workspaceDir, { recursive: true, force: true });
    } else if (workspaceDir) {
      console.log(`[${req.requestId}] keep workdir at ${workspaceDir}`);
    }
  }
});

app.listen(port, () => {
  console.log(`RoboSite listening on http://localhost:${port}`);
});
